...
export default function Sidebar() {
    ...
    // Menangani klik di luar menu pop-up
    useEffect(() => {
        ...
        <>
            ...
            <aside className='hidden md:flex w-20 h-[95vh] fixed left-5 top-1/2 -translate-y-1/2 flex-col justify-between items-center py-12'>
                ...
                <div className="relative">
                    ...
                    {/* Desktop Pop-up Menu */}
                    {elipseMenuOpen && (
                        <div ref={menuRef} className='absolute text-white bottom-0 left-16 bg-primary border border-secondary/20 shadow-2xl rounded-2xl p-2 w-48 animate-in slide-in-from-left-2 duration-200'>
                            <button className="flex cursor-pointer items-center gap-3 w-full p-3 hover:bg-secondary/10 rounded-xl text-sm transition-colors"><User size={18} /> Profile</button>
                            <button className="flex cursor-pointer items-center gap-3 w-full p-3 hover:bg-secondary/10 rounded-xl text-sm transition-colors"><Settings size={18} /> Settings</button>
                            <hr className="my-2 border border-secondary" />
                            <button className="flex cursor-pointer items-center gap-3 w-full p-3 text-red-500 hover:bg-red-500/10 rounded-xl text-sm transition-colors"><LogOut size={18} /> Logout</button>
                        </div>
                    )}
                </div>
            </aside>
            ...
            <nav className='md:hidden fixed -bottom-1 left-0 right-0 bg-secondary h-16 flex justify-around items-center px-6 z-40'>
                ...
                <div className="relative">
                    ...
                    {/* Mobile Pop-up Menu */}
                    {elipseMenuOpen && (
                        <div ref={menuRef} className='absolute bottom-20 -right-24 bg-primary text-white border border-secondary/20 shadow-2xl rounded-2xl p-2 w-48 animate-in slide-in-from-bottom-2 duration-200'>
                            ...
                            <button className="flex items-center gap-3 w-full p-3 text-red-500 hover:bg-red-500/10 rounded-xl text-sm transition-colors"><LogOut size={18} /> Logout</button>
                        </div>
                    )}
                </div>
            </nav>
